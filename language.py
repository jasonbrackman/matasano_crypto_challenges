...
class Language:
    ...
    def letter_frequency_(self):
        ...
        count = collections.Counter(letter for letter in text if letter in string.ascii_lowercase)
    ...
    def common_words(self, number=50, min_length=4):
        ...
        most_common = [key for (key, value) in counter.most_common(number) if len(key) > min_length]
        return most_common
    ...
    def part_of_language(self, decrypted: bytes, number=100, min_length=3):
        ...
        return True if any(word in decrypted.decode('utf-8') for word in english_words) else False
