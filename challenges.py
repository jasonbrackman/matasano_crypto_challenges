...
def challenge_01() -> None:
    ...
    _hex = bytes.fromhex(input_string)  # b"I'm killing your brain like a poisonous mushroom"
    _b64 = binascii.b2a_base64(_hex).decode()

    assert _b64.strip() == "SSdtIGtpbGxpbmcgeW91ciBicmFpbiBsaWtlIGEgcG9pc29ub3VzIG11c2hyb29t"
...
def challenge_07() -> None:
    ...
    with open("data/7.txt", 'r') as handle:
        ...
        assert lines[0] == "I'm back and I'm ringin' the bell ", "Decrypt AES Failed: {}".format(lines[0])
...
def challenge_11() -> object:
    ...
    def encrypt_oracle(text: bytes):
        ...
        if random.randint(1, 2) == 2:
            ...
            encrypted = cryptopals.encrypt_aes_with_custom_cbc(message, random_aes_key, random_iv)
...
def challenge_12() -> None:
    ...
    base64_decoded = binascii.a2b_base64(base64_encoded) #  base64.b64decode(base64_encoded)
    random_aes_key = cryptopals.generate_random_bytes(16)

    encrypted_blocks = cryptopals.encrypt_ecb_oracle(b'', base64_decoded, random_aes_key)

    result = cryptopals.decrypt_ecb_message_without_key(encrypted_blocks, base64_decoded, random_aes_key)
...
def challenge_14() -> object:
    ...
    encrypted_blocks = cryptopals.encrypt_ecb_oracle(b'', base64_decoded, random_aes_key, prepend=random_prepend)

    # print("Padding: {}".format(obtain_ecb_pkcs7_count(base64_decoded, random_aes_key, prepend=random_prepend)))
    cryptopals.obtain_ecb_prepend_padding_count(base64_decoded, random_aes_key, prepend=random_prepend)
    # print(decrypt_aes(b''.join(encrypted_blocks), random_aes_key))
    # print("Original Encrypted Blocks: {}".format(len(encrypted_blocks)))

    result = cryptopals.decrypt_ecb_message_without_key(encrypted_blocks, base64_decoded, random_aes_key, prepend=random_prepend)
...
def challenge_16() -> None:
    ...
    def encrypt_using_cbc(message, key):
        ...
        encrypted = cryptopals.encrypt_aes_with_custom_cbc(full_message, key, b'YELLOW SUBMARINE')

        return encrypted

    def is_admin(encrypted, random_aes_key):

        decrypted = cryptopals.decrypt_aes_with_custom_cbc(b''.join(encrypted), random_aes_key, b'YELLOW SUBMARINE')
...
def challenge_17():
    ...
    class Server:
        ...
        def get_encrypted_blocks(self):
            ...
            # encrypt using CBC
            encrypted_blocks = cryptopals.encrypt_aes_with_custom_cbc(random_string, self.key, self.iv)
        ...
        def decrypt_cookie(self, ciphertext):
            ...
            blocks = cryptopals.decrypt_aes_with_custom_cbc(ciphertext, self.key, self.iv)
...
def challenge_19() -> None:
    ...
    for index in range(0, max_line_length):
        ...
        for guess in range(256):
            ...
            items = [chr(message[index] ^ guess) for message in encrypted_messages if len(message) > index]
    ...
    for line, message in zip(lines, encrypted_messages):
        ...
        test_total_decrypt.append(cryptopals.compute_hamming_distance(b''.join(decrypt), binascii.a2b_base64(line)))

    normalized_distance = sum(test_total_decrypt) / len(test_total_decrypt)
    assert normalized_distance < 5, "Hamming Distance of {} suggests decryption failed.".format(normalized_distance)
